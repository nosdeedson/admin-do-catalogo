...
public interface VideoRepository extends JpaRepository<VideoJpaEntity, String> {
    ...
    @Query("""
            select distinct new com.E3N.admin.catalogo.domain.video.VideoPreview(
                v.id as id,
                v.title as title,
            ...
            and
                ( members.id.castMemberId in (:castMembers) or :castMembers is null )
            and
                (  categories.id.categoryId in (:categories) or :categories is null )
            and
                ( genres.id.genreId in (:genres) or :genres is null )
            """)
